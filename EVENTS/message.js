...
module.exports = async(CLIENT, message) => {
    ...
    if (message.content == `<@!${CLIENT.user.id}>` || message.content == `<@!${CLIENT.user.id}> `) {
        ...
        message.channel.send(new DISCORD.MessageEmbed()
            .setDescription("Mon prefix est " + customPrefix)
            .setFooter(`${CLIENT.user.username} • Today At ${LOGGER.embedTime}`, CLIENT.user.avatarURL()))
            }

    ///* || [PREFIX] || *///
    const PREFIXARRAY = [customPrefix, `<@!${CLIENT.user.id}>`, `<@!${CLIENT.user.id}> `, 'tk!argent'];
