...
module.exports.run = async(CLIENT, message, ARGS, PREFIXARRAY) => {
    ...
    .setTitle("**LA BOUTIQUE | ARYA **")
    .setDescription("\nAprès un achat, aucun remboursement n'est possible. \n \n **Voici les arcticles:**")
    .addField("Article 1 - Création de team **[ 5000 AR$ ]**", "Crée votre propre team, gérez la pour être le numéro 1 du serveur")
    .addField("Article 2 - Role personnalisable **[ 10000 AR$ ]**", "Grâce à votre role personnalisé, marquez vos distance avec les randoms !")
    .addField("Article 3 - Channel personnalisable **[ 15000 AR$$ ]**", "Grâce à votre channel personnalisé, vous serez le maitre des lieux !")
    .addField("Article 4 - Nitro **[ 20000 AR$ ]**", "Grâce à votre nitro, vous obtiendrez un badge personnalisé en statut ainsi qu'une pp animé et plein d'autres choses incroyables")
    )
        ...
        if(reaction._emoji.name === "4️⃣") {
            ...
            DATABASE.DB.query("SELECT * FROM coins WHERE user_id = ?", user.id, function(err, rows){
                ...
                reaction.message.guild.channels.create(`Commande n°${user.id}`, {type: "text", permissionOverwrites: [
                    ...
                    reaction.message.channel.send(`**Merci pour votre achat, cliquez sur ce salon pour finaliser votre commande [<#${channel.id}>]**`)
                    let s2 = await channel.send(new DISCORD.MessageEmbed()
                    .setTitle("**Veuillez attendre un Administrateur**")
                    .setFooter("Equire Arya | Arya Coins"))
                            }
