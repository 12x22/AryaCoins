...
module.exports.run = async(CLIENT, message, ARGS, PREFIXARRAY) => {
    ...
    DATABASE.DB.query("SELECT * FROM teams WHERE team_chief_id = ?", message.author.id, function(err, rows){
        ...
        if(rows.length < 1) {return message.channel.send(`<@${message.author.id}>, vous n'avez pas de team...`)} else {
            ...
            let target = message.mentions.members.first(); if(!target) return message.channel.send(`<@${message.author.id}>, vous devez mentionnez la personne à inviter...`)
            if(!team_role) return message.channel.send(`<@${message.author.id}>, impossible d'ajouter cet utilisateur à votre team: ERREUR INCONNU...`)
            if(!team_text) return message.channel.send(`<@${message.author.id}>, impossible d'ajouter cet utilisateur à votre team: ERREUR INCONNU...`)
            if(!team_name) return message.channel.send(`<@${message.author.id}>, impossible d'ajouter cet utilisateur à votre team: ERREUR INCONNU...`)
            ...
            DATABASE.DB.query("SELECT * FROM teams", async function(err, rows){
                ...
                if(rows.length < 1) {isTeam = false} else {
                    ...
                    if(isTeam == true) return message.channel.send(`<@${target.user.id}>, est déjà dans une team!`)
...
module.exports.help = {
    ...
    utilisations: `.inviterTeam`,
    exemples: `.inviterTeam`
}