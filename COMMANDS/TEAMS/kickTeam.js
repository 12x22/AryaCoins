...
module.exports.run = async(CLIENT, message, ARGS, PREFIXARRAY) => {
    ...
    DATABASE.DB.query("SELECT * FROM teams WHERE team_chief_id = ?", message.author.id, function(err, rows){
        ...
        if(rows.length < 1) {return message.channel.send(`<@${message.author.id}>, vous n'avez pas de team / permissions pour expulser un membre...`)} else {
            ...
            let target = message.mentions.members.first(); if(!target) return message.channel.send(`<@${message.author.id}>, vous devez mentionnez la personne à expulser...`)
            if(target.user.id == message.author.id) return message.channel.send(`<@${message.author.id}>, vous ne pouvez pas vous expluser..`)
            if(!team_role) return message.channel.send(`<@${message.author.id}>, impossible d'expulser cet utilisateur à votre team: ERREUR INCONNU...`)
            if(!team_text) return message.channel.send(`<@${message.author.id}>, impossible d'expulser cet utilisateur à votre team: ERREUR INCONNU...`)
            if(!team_name) return message.channel.send(`<@${message.author.id}>, impossible d'expulser cet utilisateur à votre team: ERREUR INCONNU...`)
            ...
            DATABASE.DB.query("SELECT * FROM teams", async function(err, rows){
                ...
                if(rows.length < 1) {isTeam = false} else {
                    ...
                    if(isTeam == false) return message.channel.send(`<@${target.user.tag}>, n'est pas dans votre team!`)

                    await target.send(new DISCORD.MessageEmbed()
                        .setTitle(`Vous avez été __explusé__ de la team: **${team_name}** sur **Arya 🔱**`)
                    )
...
module.exports.help = {
    ...
    utilisations: `.expulserTeam`,
    exemples: `.expulserTeam`
}